...
export default function TransitionsModal({ id_from_props, modal_performance }) {
  ...
  const handleEditProduct = () => {
    flag = true
    // dispatch(getBookById(id_from_props))
    axios.get('http://localhost:5000/products/' + id_from_props).then((res) => {
  ...
  useEffect(() => {
    if (flag === true) setCategory(productForEdit.category)
  }, [flag])
          ...
          <Fade in={open}>
            <div className={classes.paper}>
              ...
              <AiFillCloseCircle
                onClick={handleClose}
                size={32}
                style={{ float: 'left', fill: 'gray' }}
              />
              <h2 id="transition-modal-title">افزودن / ویرایش کالا</h2>
              <p id="transition-modal-description">
                لطفا قبل از خروج حتما دکمه ذخیره را فشار دهید
              </p>
