...
function Cart() {
  ...
  return (
    <div className={classes.root}>
      <Typography
        variant="h4"
        style={{ marginRight: '5%', marginBottom: '3%', fontWeight: '900' }}
      >
        سبد خرید
      </Typography>
      {counter === 0 ? (
        <Typography variant="h5" style={{ marginRight: '40%', color: 'gray' }}>
          سبد خرید شما خالیست
        </Typography>
      ) : (
        <Box
        ...
        >
          <TableContainer component={Paper} className={classes.table}>
            <Table>
              <TableBody>
                ...
                {myCart.map((item) => (
                  <TableRow>
                    <TableCell width="30%" className={classes.headCell}>
                      {item.title}
                    </TableCell>
          ...
          <Grid container style={{ marginRight: '30%' }}>
            <Grid item md={5} sm={6} xs={8}>
              <Typography variant="h6">جمع کل:{totalPrice}تومان</Typography>
            </Grid>
            <Grid xs={1} sm={1} md={2} />
            <Grid item md={5} sm={5} xs={6}>
              <Link
                to="/customerinfromationform"
